...
int main(int argc, char *argv[])
{
    ...
    clksim startClock;
    clksim totalClock;
    const int chanceOfTermProcess = 55;
    const int chanceOfReqResources = 50;
    int rCounter;
    int replyToOss;
    int boundB;   
    ...
    while(1)
    {
        ...
        if(boundB <= chanceOfReqResources)
        {
            ...
            resource = rand() % 20;
            replyToOss = requestToOss(resource, process, procPid);
        ...
        else if(procsResources > 0)
        {
            ...
            replyToOss = releaseToOss(resource, process, procPid);
            if(replyToOss == grantedRequest)
            {
                rCounter++;
                procsResources[resource]--;
...
int releaseToOss(int process, int procPid, int resource)
{
    int sendmessage, receivemessage;
    /* Send the message to oss with type 1 and it's a request */
    msg message = {.typeofMsg = 1, .msgDetails = releaseResource, .resource = resource, .process = process, .sendingProcess = procPid};
...
void terminateToOss(int process, int procPid)
{
    int sendmessage, receivemessage;
    /* Send the message to oss with type 1 and it's a request */
    msg message = {.typeofMsg = 1, .msgDetails = terminateProcess, .process = process, .sendingProcess = procPid};
